...
export function CRXtoZIP(arraybuffer: ArrayBuffer): ArrayBuffer {
...
function getPublicKeyFromProtoBuf(bytesView: Uint8Array, startOffset: number, endOffset: number): string {
  ...
  const crxIdHex = enc.Latin1.parse(getBinaryString(crxIdBin, 0, 16)).toString();
  for (let i = 0; i < publicKeys.length; ++i) {
    const sha256sum = SHA256(enc.Latin1.parse(publicKeys[i])).toString();
